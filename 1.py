...
parser = argparse.ArgumentParser(
    description='Handle MOFs cif and remove solvent atom(s)')
...
class rmsolvent:
    ...
    def seek_crystal(self, cf):
        ...
        cell_parameters_re = re.compile(
            r'(?P<name>_cell_[l|a].*?)\s+?(?P<value>.*)')
    ...
    def get_cart(atom_array_cd, cell):
        ...
        frac_a, frac_b, frac_c = atom_array_cd[0], \
            atom_array_cd[1], \
            atom_array_cd[2]
        x_cart = lx_vector[0] * frac_a + \
            ly_vector[0] * frac_b + lz_vector[0] * frac_c
        y_cart = lx_vector[1] * frac_a + \
            ly_vector[1] * frac_b + lz_vector[1] * frac_c
        z_cart = lx_vector[2] * frac_a + \
            ly_vector[2] * frac_b + lz_vector[2] * frac_c
        ...
        for dt in dt_list:
            ...
            for radius_dict in radius_list:
                ...
                if atom_a == atom_species:
                    radius_a = float(atom_radius)
                if atom_b == atom_species:
                    radius_b = float(atom_radius)
    ...
    def same_atom(have_bond_dict):
        for bond_type, value in have_bond_dict.items():
            atom_a_site, atom_b_site = bond_type.split(
                '-')[0],  bond_type.split('-')[2]
    ...
    def connect_network(self, have_bond_list, atom_cd_fra):
        ...
        for atom_chain in cn_atom_chain:
            cn_chain_dict = {}
            # print(atom_chain)
            per_cn_species = rmsolvent.get_cn_atom_species(
                atom_chain, atom_cd_fra)
            per_cn_species.sort()
