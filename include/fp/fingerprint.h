...
#include "../blaswrap.h"
    ...
    extern int dsyev_(char *jobz, char *uplo, long *n, double *a, long *lda,
                      double *w, double *work, long *lwork, long *info);

    extern int dsygv_(long *itype, char *jobz, char *uplo, long *n,
           ...
           double *w, double *work, int *lwork, int *info)
{
    long ln = (long)(*n);
    long llda = (long)(*lda);
    long llwork = (long)(*lwork);
    long linfo = (long)(*info);

    dsyev_(jobz, uplo, &ln, a, &llda, w, work, &llwork, &linfo);
...
void get_fp(int flag, int nat, int ntyp, int ixyz, int nx, int lseg, int l, double lat[3][3],
            double rxyz[][3], int types[], double rcov[], double cutoff, double **lfp, double **sfp, bool print);
...
void get_fp(int flag, int nat, int ntyp, int ixyz, int nx, int lseg, int l, double lat[3][3],
            double rxyz[][3], int types[], double rcov[], double cutoff, double **lfp, double **sfp, bool print = false)
{
                    ...
                    for (iz = -ixyz; iz <= ixyz; iz++)
                    {
                        ...
                        if (d2 <= cutoff2)
                        {
                            ...
                            if (n_sphere > nx)
                            {
                                ...
                                // exit(1);
                                throw Exception("Error: cutoff is too large.");
                            }
    ...
    { /* s orbital only */
        for (iat = 0; iat < n_sphere; iat++)
        {
            ...
            for (jat = 0; jat < n_sphere; jat++)
            {
                ...
                om[iat][jat] = pow(sqrt(4.0 * r * (rcov_sphere[iat] * rcov_sphere[jat])), 3) * exp(-d2 * r) * amp[iat] * amp[jat];
            }
        }
    }
    else
    { /* both s and p orbital */
        for (iat = 0; iat < n_sphere; iat++)
        {
            xi = rxyz_sphere[iat][0];
            yi = rxyz_sphere[iat][1];
            ...
            for (jat = 0; jat < n_sphere; jat++)
            {
                ...
                r = 0.5 / (rcov_sphere[iat] * rcov_sphere[iat] + rcov_sphere[jat] * rcov_sphere[jat]);
                om[4 * iat][4 * jat] = pow(sqrt(4.0 * r * (rcov_sphere[iat] * rcov_sphere[jat])), 3) * exp(-d2 * r) * amp[iat] * amp[jat];
...
int get_ixyz(double lat[3][3], double cutoff)
{
    ...
    if (info > 0)
    {
        ...
        // exit(1);
        throw Exception("Error: DSYEV ixyz");
    }
