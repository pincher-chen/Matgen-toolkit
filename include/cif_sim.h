...
double get_fp_similarity(CIF &a, CIF &b) {
    double cutoff = 0.1;
    int lmax = 0, lseg, l, nat;
    int natx = 100, ntype = elements.size();;
    ...
    for(auto iter = a_atom_cd.begin(); iter != a_atom_cd.end(); iter++, i++) {
        ...
        a_types[i] = distance(elements.begin(), atom);
    }
    ...
    for(auto iter = b_atom_cd.begin(); iter != b_atom_cd.end(); iter++, i++) {
        ...
        b_types[i] = distance(elements.begin(), atom);
    }
    ...
    get_fp_periodic(lmax, nat, ntype, a_types, a_lattice, a_position, znucl, natx, cutoff, a_sfp, a_lfp);
    get_fp_periodic(lmax, nat, ntype, b_types, b_lattice, b_position, znucl, natx, cutoff, b_sfp, b_lfp);

    auto s = a.get_atom_cd_set();
    int f[s.size()];
    double dis = get_fpdistance_periodic(nat, ntype, a_types, natx * lseg, a_lfp, b_lfp, f);
